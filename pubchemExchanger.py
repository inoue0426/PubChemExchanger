...
def get_identifiers(data, key):
    return data['IdentifierList'].get(key, []) if data and 'IdentifierList' in data else []

def get_synonyms(data, prefix):
    return [syn for syn in data['InformationList']['Information'][0]['Synonym'] if syn.startswith(prefix)] if data else []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Retrieve information about a drug from PubChem')
    parser.add_argument('-d', '--drug_name', type=str, help='The name of the drug to query')
